...
class MyRNN(keras.Model):
    def __init__(self,units):
        ...
        self.state0 = [tf.zeros([batchsz,64])]
        self.state1 = [tf.zeros([batchsz, 64])]
        #transform text to embedding representation[b,80]=>[b,80,100]
        self.embeding = tf.keras.layers.Embedding(total_words,embedding_len,input_length=max_review_len)#(所有词数（样本种类），每个样本的特征维度，最大语句长度（考虑之前最长的样本长度）)
        #cell [b,80,100],h_dim:64
        self.cell1 = layers.SimpleRNNCell(units,dropout=0.5)
        self.cell2 = layers.SimpleRNNCell(units, dropout=0.5)
